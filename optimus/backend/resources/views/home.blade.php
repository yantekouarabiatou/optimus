...
{{-- ── À PROPOS ─────────────────────────────────────────────────────────────── --}}
<section id="apropos" class="py-20 lg:py-32 bg-background">
  <div class="container mx-auto px-4 lg:px-8">
    <div class="grid lg:grid-cols-2 gap-16 items-center">
      <div data-aos="fade-right">
        <span class="inline-block px-4 py-1.5 bg-accent/10 text-accent rounded-full text-sm font-medium mb-6">À propos</span>
        <h2 class="text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-6 leading-tight">
          L'innovation technologique<br><span class="text-accent">au cœur de l'Afrique</span>
        </h2>
...
{{-- ── SERVICES ─────────────────────────────────────────────────────────────── --}}
<section id="services" class="py-20 lg:py-32 bg-muted/30">
  <div class="container mx-auto px-4 lg:px-8">
...
<section class="py-20 lg:py-32 bg-primary relative overflow-hidden">
  ...
  <div class="container mx-auto px-4 lg:px-8 relative">
    ...
    <div class="grid md:grid-cols-3 gap-6">
      @foreach([['Innovation','Nous adoptons les technologies les plus avancées pour créer des solutions novatrices adaptées au contexte africain.','💡'],['Excellence','Chaque projet est traité avec le plus haut niveau de professionnalisme et d\'expertise technique.','🏆'],['Impact','Notre objectif est de créer un impact économique et social positif durable pour nos clients.','🚀']] as [$title, $desc, $emoji])
        <div data-aos="zoom-in" data-delay="{{ $loop->index * 150 }}" class="bg-white/5 border border-white/10 rounded-2xl p-8 hover:bg-white/10 transition-colors">
          <div class="text-4xl mb-4">{{ $emoji }}</div>
          <h3 class="text-2xl font-bold text-white mb-3">{{ $title }}</h3>
          <p class="text-white/60 text-base leading-relaxed text-justify">{{ $desc }}</p>
        </div>
      @endforeach
    </div>
  </div>
...
{{-- ── ÉQUIPE ───────────────────────────────────────────────────────────────── --}}
<section id="equipe" class="py-20 lg:py-32 bg-background">
  <div class="container mx-auto px-4 lg:px-8">
    ...
    @php
      $team = [
        ...
        ['Nassirou KPASSO','Directeur / Responsable Marketing & Support Client','Cotonou','Marketing Digital · Support Client · Communication','/NK.jpeg','from-rose-500 to-pink-700',null,'https://wa.me/22994199760'],
      ];
    @endphp

    <div x-data="{ lightbox: null }" class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
      @foreach($team as [$name, $role, $location, $specialty, $photo, $gradient, $linkedin, $whatsapp])
        <div data-aos="fade-up" data-delay="{{ $loop->index * 100 }}" class="group bg-card rounded-3xl border border-border overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
          <button type="button" @click="lightbox = {{ json_encode(compact('name','role','location','photo','gradient')) }}"
                  class="relative h-52 w-full bg-gradient-to-br {{ $gradient }} overflow-hidden block focus:outline-none">
            <img src="{{ $photo }}" alt="{{ $name }}" class="absolute inset-0 w-full h-full object-cover object-top transition-transform duration-500 group-hover:scale-105">
          ...
          <div class="p-5">
            <h3 class="font-bold text-foreground text-lg mb-0.5">{{ $name }}</h3>
            <p class="text-accent text-base font-medium mb-1">{{ $role }}</p>
            <div class="flex items-center gap-1 mb-3">
              <div class="w-1.5 h-1.5 bg-green-500 rounded-full"></div>
              <span class="text-sm text-muted-foreground">{{ $location }}</span>
            </div>
            <p class="text-sm text-muted-foreground border-t border-border pt-3">{{ $specialty }}</p>
            <div class="flex gap-2 mt-4">
              <a href="#contact" class="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-accent border border-border rounded-lg py-2 hover:border-accent/50 transition-colors">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                Contact
              </a>
              @if($linkedin)
                <a href="{{ $linkedin }}" target="_blank" rel="noopener noreferrer" class="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-[#0A66C2] border border-border rounded-lg py-2 hover:border-[#0A66C2]/50 transition-colors">
              ...
              @elseif($whatsapp)
                <a href="{{ $whatsapp }}" target="_blank" rel="noopener noreferrer" class="flex-1 flex items-center justify-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-[#25D366] border border-border rounded-lg py-2 hover:border-[#25D366]/50 transition-colors">
...
{{-- ── CONTACT ──────────────────────────────────────────────────────────────── --}}
<section id="contact" class="py-20 lg:py-32 bg-muted/30">
  <div class="container mx-auto px-4 lg:px-8">
    <div data-aos="fade-up" class="text-center max-w-2xl mx-auto mb-16">
    ...
    <div class="grid lg:grid-cols-2 gap-12 max-w-7xl mx-auto">
      ...
      {{-- Info --}}
      <div class="space-y-6">
        @foreach([['📍','Adresse','Porto-Novo & Cotonou, Bénin, Afrique de l\'Ouest'],['📞','Téléphone','+229 01 40 84 19 00'],['✉️','Email','contact@optimussolutions.com'],['🕒','Horaires','Lun – Ven : 8h00 – 18h00']] as [$emoji, $label, $value])
...
<div x-data="{
  ...
  <div x-show="open" x-cloak
       ...
       class="mb-4 w-[440px] bg-card border border-border rounded-3xl shadow-2xl overflow-hidden">
    ...
    <div x-ref="msgs" class="h-64 overflow-y-auto p-4 space-y-3 chatbot-messages">
    ...
    <div x-show="messages.length <= 1" class="px-4 py-3 border-t border-border bg-muted/40">
      <p class="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-2.5">Questions fréquentes</p>
      <div class="flex flex-wrap gap-2">
        <template x-for="q in ['🛠️ Quels sont vos services ?','📞 Comment vous contacter ?','🛒 Voir la boutique','💰 Demander un devis','🌍 Où êtes-vous situés ?']">
    ...
    <div class="p-4 border-t border-border flex gap-2">
      <input x-model="input" @keydown.enter="send()" type="text" placeholder="Posez votre question..."
             class="flex-1 px-3 py-2.5 bg-background border border-input rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ring/50">
      <button @click="send()" class="px-4 py-2.5 bg-primary text-white rounded-xl hover:bg-primary/90 transition-colors">
  ...
  <button @click="open=!open"
          class="w-20 h-20 bg-primary hover:bg-primary/90 text-white rounded-full shadow-2xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 ring-4 ring-primary/20">
    <svg x-show="!open" class="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
    <svg x-show="open" x-cloak class="w-9 h-9" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
  </button>
</div>
